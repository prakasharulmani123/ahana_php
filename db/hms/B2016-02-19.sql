...
insert  into `co_super_admin`(`su_id`,`username`,`password`,`authtoken`,`last_activity`,`created_by`,`created_at`,`modified_by`,`modified_at`) values (1,'demo','$2y$13$pfNu2lorSgd3btV7foqp.uriRfXF.7lSi51b45oefB0V1NaObxEui','ZGVtbzE0NTU3MTE2ODI4MDcz','0000-00-00 00:00:00',NULL,'0000-00-00 00:00:00',-1,'2016-01-07 11:46:28');
...
/*Table structure for table `pat_patient_tenant` */

DROP TABLE IF EXISTS `pat_patient_tenant`;

CREATE TABLE `pat_patient_tenant` (
  ...
  KEY `FK_pat_patient_tenant_patient` (`patient_id`),
  KEY `FK_pat_patient_tenant_tenant` (`tenant_id`),
  CONSTRAINT `FK_pat_patient_tenant_patient` FOREIGN KEY (`patient_id`) REFERENCES `pat_patient` (`patient_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_pat_patient_tenant_tenant` FOREIGN KEY (`tenant_id`) REFERENCES `co_tenant` (`tenant_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `pat_patient_tenant` */
