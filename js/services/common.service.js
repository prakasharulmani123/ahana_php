...
function CommonService($http, $rootScope, $window, $q, $filter, $localStorage, AuthenticationService) {
    ...
    service.GetEncounterListByPatientAllTenant = GetEncounterListByPatientAllTenant;
...
//        $http.get($rootScope.IRISOrgServiceUrl + '/chargepersubcategory/getroomchargesubcategorylist?deleted=' + del_sts + '&cat_id=' + cat_id)
    ...
    function GetEncounterListByPatientAllTenant(sts, del_sts, pat_id, pat_type, callback) {
        var response;

        $http.get($rootScope.IRISOrgServiceUrl + '/encounter/getencounterlistbypatientdifftenant?status=' + sts + '&deleted=' + del_sts + '&patient_id=' + pat_id + '&encounter_type=' + pat_type)
                .success(function (response) {
                    callback(response);
                }, function (x) {
                    response = {success: false, message: 'Server Error'};
