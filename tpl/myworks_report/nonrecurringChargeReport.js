        ...
        $scope.loadReport = function () {
            ...
            $http.post($rootScope.IRISOrgServiceUrl + '/patientbillingothercharge/getcharges', data)
                    .success(function (response) {
                        ...
                        $scope.othercharges = response.billingOthercharges;
                        $scope.consultant = response.consultant;
                        $scope.procedure = response.procedure;
                        $scope.tableid = [];
                        $scope.sheet_name = [];
                        if (response.billingOthercharges.length > 0) {
                            $scope.sheet_name.push('Other Charges');
                            $scope.tableid.push('table_othercharges');
                        }
                        if (response.consultant.length > 0) {
                            $scope.sheet_name.push('Professional Charges');
                            $scope.tableid.push('table_professional');
                        }
                        if (response.procedure.length > 0) {
                            $scope.sheet_name.push('Procedure Charges');
                            $scope.tableid.push('table_procedure');
                        }
