app.controller('ResultsController', ['$rootScope', '$scope', '$timeout', '$http', '$state', 'modalService', '$filter', function ($rootScope, $scope, $timeout, $http, $state, modalService, $filter) {
        ...
        $scope.initCanCreateResult = function () {
...
//        //Index Page
        $scope.enabled_dates = [];
        $scope.loadPatResultsList = function (date) {
...
//                    encounter_id: $scope.encounter.encounter_id
                ...
                post_url = $rootScope.IRISOrgServiceUrl + '/patientresults/' + _that.data.pat_result_id;
                method = 'PUT';
            ...
            $http({
                ...
                data: _that.data,
                    ...
                    function (response) {
                        ...
                        $scope.data = {};
                        $timeout(function () {
                            $state.go('patient.results', {id: $state.params.id});
                        }, 1000)
        ...
        $scope.loadForm = function () {
            ...
            $scope.errorData = "";
            $http({
                url: $rootScope.IRISOrgServiceUrl + "/patientresults/" + $state.params.result_id,
                method: "GET"
            }).success(
                    function (response) {
                        $scope.loadbar('hide');
                        if ($scope.data.formtype == 'update') {
                            $scope.initCanCreateResult();
                        }
                        $scope.data = response;
        ...
        //Delete
        $scope.removeRow = function (result_id) {
            var modalOptions = {
                closeButtonText: 'No',
